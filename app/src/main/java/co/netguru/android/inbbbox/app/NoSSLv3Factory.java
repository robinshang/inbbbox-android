...
public class NoSSLv3Factory extends SSLSocketFactory {
    ...
    private static final String OPEN_SSL_PACKAGE = "org.apache.harmony.xnet.provider.jsse.OpenSSLSocketImpl";
    private static final String METHOD_NAME = "setUseSessionTickets";
    ...
    private static Socket makeSocketSafe(Socket socket) {
        if (socket instanceof SSLSocket) {
            ((SSLSocket) socket).setEnabledProtocols(new String[]{TLS_1_1, TLS_1_2});
    ...
    private static class NoSSLv3SSLSocket extends DelegateSSLSocket {
        ...
        private NoSSLv3SSLSocket(SSLSocket delegate) {
            ...
            if (!canonicalName.equals(OPEN_SSL_PACKAGE)) {
                try {
                    Method msetUseSessionTickets = delegate.getClass().getMethod(METHOD_NAME, boolean.class);
        ...
        @Override
        public void setEnabledProtocols(String[] protocols) {
            if (protocols != null && protocols.length == 1 && SSL_3.equals(protocols[0])) {
                List<String> enabledProtocols = new ArrayList<>(Arrays.asList(delegate.getEnabledProtocols()));
                if (enabledProtocols.size() > 1) {
                    enabledProtocols.remove(SSL_3);
                }
